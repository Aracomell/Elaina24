...
async function startelaina() {
    ...
    const elaina = elainaConnect({
        ...
        browser: ['SkylarKaf','Safari','1.0.0'],
        auth: state,
        version
    })
    ...
    elaina.sendMessage(callerId, { text: `Automatic block system!\nDon't call bot!\nPlease contact the owner to open !`}, { quoted : pa7rick })
    elaina.sendMessage(`6282331660134@s.whatsapp.net`, {text: `*Report Bot:* Someone Called Bot`})
    await sleep(8000)
...
Don't forget to obey the rules in this group!`} )
                } else if (anu.action == 'remove') {
                    elaina.sendMessage(anu.id, { image: Tol, contextInfo: { mentionedJid: [num] }, caption: `@${num.split("@")[0]} Leaving From :
${metadata.subject}
...
//setting
	...
	for (let i of kon) {
	    list.push({
	    	...
	    	vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await elaina.getName(i + '@s.whatsapp.net')}\nFN:${await elaina.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Click To Chat\nitem2.EMAIL;type=INTERNET:Gmail: skylarkaf7@gmail.com\nitem2.X-ABLabel:If you have Question\nitem3.URL:https://s.id/Skylarkaf\nitem3.X-ABLabel:Youtube\nitem4.ADR:;;Indonesia, Jatim;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
	    })
	}
	elaina.sendMessage(jid, { contacts: { displayName: `${list.length} Contact`, contacts: list }, ...opts }, { quoted })
    }
